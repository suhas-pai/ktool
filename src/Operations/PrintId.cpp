...
namespace Operations {
    ...
    PrintId::run(const Objects::MachO &MachO) const noexcept -> RunResult {
        ...
        const auto LoadCommandsMap =
            ::MachO::LoadCommandsMap(LoadCommandsMemoryMap,
                                     IsBigEndian);

        for (auto It = LoadCommandsMap.begin();
             It != LoadCommandsMap.end();
             It++)
        {
            const auto &LoadCommand = *It;
            if (const auto ID =
    ...
    RunResult PrintId::run(const Objects::Base &Base) const noexcept {
        switch (Base.kind()) {
            case Objects::Kind::None:
